...
module.exports = (bot, {viewDistance = 6, firstPerson = false, port = 3000, prefix = ''}) => {
  ...
  io.on('connection', (socket) => {
    ...
    function botPosition() {
      // HACKS for viewing the bot from an offset
      const packet = {pos: bot.entity.position, yaw: bot.entity.yaw, addMesh: true}
      if (firstPerson) {
        const target = bot.entity.position.offset(0, 1.6, 0)
        const perspective = new Vec3(0, 5 - 1.6, 5).unit()
        packet.pos = bot.entity.position.offset(0, 5, 5)
        packet.pitch = Math.asin(-1 * perspective.y)
        packet.yaw = Math.atan2(perspective.x, perspective.z)
